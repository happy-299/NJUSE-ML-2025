...
def plot_training_history(history_path: str, save_dir: Optional[str] = None):
    ...
    # 损失曲线
    axs[0, 0].plot(epochs, train_loss, 'b-', label='训练损失')
    axs[0, 0].plot(epochs, val_loss, 'r-', label='验证损失')
    axs[0, 0].set_title('训练和验证损失')
...
def plot_model_comparison(results_dirs: List[str], 
    ...
    reg_data = {metric: [r['test_reg'][metric] for r in results] for metric in reg_metrics}
    cls_data = {metric: [r['test_cls'][metric] for r in results] for metric in cls_metrics}
