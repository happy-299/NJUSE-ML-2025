...
| 成员 | 分工与贡献 |
|------|------|
| 肖继鹏 | 模型组件消融实验、泛化性分析、假设检验评估模型、文档整理和本报告产出 |
| 何浩川   | 数据获取、数据预处理、特征工程、特征消融实验 |
| 伍子科 | 两种模型的建模、训练 |
| 张涛 | 两种模型的建模、训练 |
---
...
| 禁用-文本语义   | 0.618380 | 2265.77 | 697.572 | 5.13373e+06 |
...
| 特征集          | Accuracy | Precision |   Recall |       F1 | F1_macro |
| --------------- | -------: | --------: | -------: | -------: | -------: |
...
| 禁用-文本语义   | 0.694150 |  0.651255 | 0.797090 | 0.716830 | 0.692175 |
| 禁用-评审协作   | 0.828426 |  0.873134 | 0.756669 | 0.810741 | 0.826914 |
| 禁用-作者经验   | 0.783274 |  0.731889 | 0.873888 | 0.796610 | 0.782339 |
| 禁用-网络中心性 | 0.886141 |  0.940465 | 0.817300 | 0.874567 | 0.885163 |
| 禁用-项目上下文 | 0.828426 |  0.873134 | 0.756669 | 0.810741 | 0.826914 |
| 禁用-时间模式   | 0.888496 |  0.902790 | 0.863379 | 0.882645 | 0.888218 |
...
为进一步分析多任务模型各组件对性能的影响，分别对 Shared-Bottom 和 MMoE 模型进行了消融实验，主要包括：
- 去除 Dropout（no_dropout）
- 塔结构变浅（shallow_tower）
- 降低 Embedding 维度（small_embedding）
- MMoE 模型额外包含：减少专家数量（fewer_experts）、单专家（single_expert）

**消融实验结果**：

| 模型/消融项        |        R² |       RMSE |       MAE |  Accuracy |        F1 |
| ------------------ | --------: | ---------: | --------: | --------: | --------: |
| Shared-Bottom 原始 |     0.615 |     2275.8 |     766.4 |     0.813 |     0.780 |
| 无Dropout          |     0.601 |     2316.1 |     713.9 |     0.692 |     0.711 |
| 浅层塔             |     0.631 |     2229.3 |     814.1 |     0.737 | **0.751** |
| 小Embedding        | **0.699** | **2013.4** | **543.0** |     0.766 |     0.735 |
| MMoE 原始          |     0.552 |     2454.8 |     710.9 |     0.830 |     0.804 |
| 无Dropout          |     0.554 |     2450.5 |     715.9 |     0.802 |     0.772 |
| 浅层塔             |     0.570 |     2406.1 |     685.9 | **0.863** | **0.840** |
| 小Embedding        | **0.694** | **2029.1** | **547.0** | **0.870** | **0.860** |
| 减少专家数量       |     0.575 |     2390.3 |     771.6 |     0.740 |     0.713 |
| 单专家             |     0.588 |     2353.7 |     717.0 |     0.711 |     0.691 |

> 注：表中加粗项标注了在消融实验中最显著的性能变化（如Embedding变小的显著提升、浅层塔/专家数变化对分类的影响等），所有数值均来自 outputs/ablation_shared 和 outputs/ablation_mmoe 的 ablation_summary.json 文件。

**主要发现**：
- 降低 Embedding 维度对回归和分类性能提升最明显（Shared-Bottom R² 从 0.615 提升到 **0.699**；MMoE R² 从 0.552 提升到 **0.694**，对应 RMSE/MAE 明显下降）。
- 塔结构变浅在分类任务上带来提升（Shared-Bottom F1=**0.751**，MMoE F1=**0.840**），但对回归的影响不一致，部分情况下回归误差反而增加。
- 去除 Dropout（no_dropout）通常会使分类性能下降（F1 或 Accuracy 降低），提示 Dropout 在泛化上仍有作用。
- 对于 MMoE，减少专家数量（fewer_experts）或使用单专家（single_expert）均导致性能下降，说明专家机制对模型性能有重要贡献。

**消融实验可视化**：

![Ablation Comparison](outputs/figures_level2/ablation_comparison.png)
*图: 多任务模型组件消融实验结果对比*
...
3. **与单项目对比**:
   ...
   - **发现**: Shared-Bottom 在单项目和跨项目上保持回归优势，但跨项目场景下优势缩小，说明 MMoE 的专家机制在多样化数据上更有价值
