...
class MTLModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.embedding = EmbeddingBlock(cat_cardinalities, embedding_dim, dropout)
        fused_input_dim = numeric_dim + (cat_onehot_dim if self.use_one_hot else self.embedding.output_dim)

        self.wide = nn.Linear(fused_input_dim, 32) if fused_input_dim > 0 else None
        self.deep_input = BottomMLP(fused_input_dim, bottom_mlp, dropout) if bottom_mlp else None
        ...
        if model_type.lower() == 'mmoe':
            self.core = MMoE(combined_dim, experts, expert_hidden, tower_hidden_reg, tower_hidden_cls, dropout)
        elif model_type.lower() == 'shared_bottom':
            self.core = SharedBottom(combined_dim, [combined_dim], tower_hidden_reg, tower_hidden_cls, dropout)
        else:
            raise ValueError(f"不支持的模型类型: {model_type}")

    def forward(self, x_num: Optional[torch.Tensor], x_cat: Optional[torch.Tensor], x_cat_onehot: Optional[torch.Tensor] = None):
        parts = []
        ...
        elif x_cat is not None and x_cat.numel() > 0 and x_cat.dtype.is_floating_point:
            parts.append(x_cat)
        else:
            emb = self.embedding(x_cat)
            if emb is not None and emb.numel() > 0:
                parts.append(emb)
        x = torch.cat(parts, dim=-1) if parts else torch.empty(0)
